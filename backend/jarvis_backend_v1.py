...
class FileAgent:
    ...
    def _search_item(self, **kwargs):
        ...
        max_depth = kwargs.get("max_depth", None)  # Optional: limit recursion depth
        ...
        try:
            ...
            # Perform recursive search using os.walk
            for root, dirs, files in os.walk(search_path):
                # Optional: Implement depth limitation
                if max_depth is not None:
                    current_depth = root.replace(search_path, '').count(os.sep)
                    if current_depth > max_depth:
                        # Remove directories to prevent going deeper
                        dirs[:] = []
                        continue
